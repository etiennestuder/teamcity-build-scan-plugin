...
final class DefaultBuildScanDisplayArbiter implements BuildScanDisplayArbiter {
    ...
    static final String MAVEN_RUNNER = "Maven2"; // TODO test
    ...
    public boolean showBuildScanInfo(@NotNull SBuild build) {
        ...
        if (runnerTypes.contains(GRADLE_RUNNER)) {
            return true;
        } else if ((runnerTypes.contains(SIMPLE_RUNNER) || runnerTypes.contains(MAVEN_RUNNER)) && !buildScanLookup.getBuildScansForBuild(build).isEmpty()) {
            return true;
        } else {
            return false;
        }
